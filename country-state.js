$(document).ready(function () {
    ...
    data.data.forEach(function (country) {
        if (country.states.length > 0) {
            let countryCode = country.iso3.slice(0, 2);


            $countryDropdown.append(
                `<option value="${countryCode}" data-index="${data.data.indexOf(country)}">${country.name}</option>`
            );
        }
    });

    $countryDropdown.change(function () {
        let selectedIndex = $(this).find(':selected').data('index');
        let selectedCountry = data.data[selectedIndex];
        if (selectedCountry && selectedCountry.states.length > 0) {

            $stateDropdown.prop('disabled', false).empty().append('<option value="">Select State</option>');
